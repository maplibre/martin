...
const SpriteCanvas = ({meta,image,label,previewMode = false}: SpriteCanvasProps) => {
      ...
      <div className="flex flex-col items-center justify-center m-1.5 h-7 w-7">
        {(!meta || !image) ? (
          ...
          <Tooltip>
            ...
            <TooltipContent>
              <p>Sprite: <code className="bg-purple-200 font-semibold font-monospace text-purple-950 p-1 rounded-sm">{label}</code></p>
            </TooltipContent>
          </Tooltip>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center m-4 h-32 w-24">
      ...
      <code className="text-monospace text-gray-700 break-all text-center mt-2">
        {label}
      </code>
    </div>
  );
