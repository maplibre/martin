...
jest.mock("@/components/ui/disabledNonInteractiveButton", () => ({
	DisabledNonInteractiveButton: ({
		children,
		...props
	}: MockComponentProps) => <button {...props}>{children}</button>,
}));
...
describe("StylesCatalog Component", () => {
	...
	it("renders loading skeleton when isLoading is true", () => {
		...
		expect(
			screen.getByText("Preview all available map styles and themes"),
		).toBeInTheDocument();
	...
	it("renders error state when there is an error", () => {
		...
		expect(
			screen.getByText("Unable to fetch style catalog from the server"),
		).toBeInTheDocument();
	...
	it("shows no results message when search has no matches", () => {
		...
		expect(
			screen.getByText("Learn how to configure Styles"),
		).toBeInTheDocument();
