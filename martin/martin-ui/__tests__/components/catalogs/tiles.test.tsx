...
describe("TilesCatalog Component", () => {
  ...
  it("matches snapshot for loading state", () => {
    const { asFragment } = render(<TilesCatalog {...defaultProps} isLoading={true} />);
    expect(asFragment()).toMatchSnapshot();
  });

  it("matches snapshot for loaded state with mock data", () => {
    const { asFragment } = render(<TilesCatalog {...defaultProps} />);
    expect(asFragment()).toMatchSnapshot();
  });
  ...
  it("renders error state when there is an error", () => {
    ...
    expect(screen.getByTestId("error-title").textContent).toBe("Failed to Load Tiles Catalog");
  });
  ...
  it("renders tile sources correctly", () => {
    ...
    expect(descriptions.some((el) => el.textContent?.includes("Test Source 1"))).toBe(true);
    expect(descriptions.some((el) => el.textContent?.includes("Test Source 2"))).toBe(true);
    expect(descriptions.some((el) => el.textContent?.includes("Test Source 3"))).toBe(true);
  });
  ...
  it("shows no results message when search has no matches", () => {
    ...
    expect(screen.getByText(/No tile sources found matching "nonexistent"/i)).toBeInTheDocument();
