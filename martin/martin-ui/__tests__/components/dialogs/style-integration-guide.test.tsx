...
describe("StyleIntegrationGuideDialog Component", () => {
  ...
  it("shows style information section", () => {
    ...
    expect(screen.getByText("http://localhost:3000/style/test-style")).toBeInTheDocument();
  ...
  it("displays web integration examples in MapLibre GL JS tab", () => {
    ...
    expect(screen.getByText("Web Browser (CDN) - HTML + JavaScript")).toBeInTheDocument();
  ...
  it("renders the active tab panel", () => {
    ...
    expect(screen.getByRole("tabpanel", { name: /maplibre gl js/i })).toBeInTheDocument();
  });
  ...
  it("shows additional resources section", () => {
    ...
    expect(screen.getByText("MapLibre Style Specification")).toBeInTheDocument();
  ...
  it("includes style URL in code examples", () => {
    ...
    expect(screen.getAllByText(/http:\/\/localhost:3000\/style\/test-style/)).toHaveLength(4); // Should appear in URL display + 3 code examples
  });
});
