...
describe("ErrorState Component", () => {
	...
	it("renders with custom title and description", () => {
		const { container } = render(
			<ErrorState
				...
				description="Custom error description for testing"
			/>,
		);
		expect(container).toMatchSnapshot();
		expect(screen.getByText("Custom Error Title")).toBeInTheDocument();
		expect(
			screen.getByText("Custom error description for testing"),
		).toBeInTheDocument();
	});
	...
	it("renders error details when showDetails is true", () => {
		...
		const { container } = render(
			<ErrorState error={errorMessage} showDetails={true} />,
		);
		expect(container).toMatchSnapshot();
	...
	it("shows retrying state when isRetrying is true", () => {
		const handleRetry = jest.fn();
		const { container } = render(
			<ErrorState onRetry={handleRetry} isRetrying={true} />,
		);
		expect(container).toMatchSnapshot();
...
describe("InlineErrorState Component", () => {
	it("renders generic inline error state correctly", () => {
		const { container } = render(
			<InlineErrorState message="Something went wrong" />,
		);
		expect(container).toMatchSnapshot();
		expect(screen.getByText("Something went wrong")).toBeInTheDocument();
	});
