...
type Glyph = { bitmap: Uint8Array; width: number; height: number; left: number; top: number; advance: number };

function parseGlyphs(data: ArrayBuffer) {
  const glyphs = new Map<number, Glyph>();
  new Pbf(data).readFields((tag, _, pbf) => {
    if (tag === 1) pbf.readMessage((tag, _, pbf) => {
      if (tag === 3) {
...
function render(canvas: HTMLCanvasElement, glyphs: Map<number, Glyph>) {
  ...
  let tw = 0, minY = Infinity, maxY = -Infinity;
  ...
  for (const c of PREVIEW) {
    ...
    if (g.width > 0 && g.height > 0) {
      const bw = g.width + 6, bh = g.height + 6;
