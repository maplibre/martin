...
/// A malformed connection string written directly in the config must (1) never echo the
/// password into the diagnostic and (2) point at the offending line.
#[cfg(feature = "postgres")]
#[test]
fn malformed_connection_string_redacts_password_and_points_at_line() {
    let yaml = "postgres:\n  connection_string: postgres://user:hunter2@bad???host/db\n";
    let stderr = run_with_bad_config(yaml, &[]);
    assert!(
    ...
    assert!(
        stderr.contains("****"),
        "expected the redaction marker in diagnostic:\n{stderr}"
    );
    insta::assert_snapshot!(stderr.trim(), @"
    martin::config::postgres::invalid_connection_string (https://maplibre.org/martin/config-file/)
