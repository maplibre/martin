...
fn compile_protos() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !std::path::Path::new("src/geojson/mvt/vector_tile.rs").exists() {
        let mut config = prost_build::Config::new();
        config.out_dir("src/geojson/mvt");
        config.compile_protos(
            &["src/geojson/mvt/vector_tile.proto"],
            &["src/geojson/mvt/"],
        )?;
        // read file contents to string
        let mut file = std::fs::OpenOptions::new()
            .read(true)
            .open("src/geojson/mvt/vector_tile.rs")?;
