...
use crate::config::file::{
    ConfigExtras, ConfigFileError, ConfigFileResult, SourceConfigExtras, UnrecognizedKeys, UnrecognizedValues,
};
...
impl ConfigExtras for PmtConfig {
    fn init_parsing(&mut self, cache: OptMainCache) -> ConfigFileResult<()> {
        if self.cache.is_some() {
            return Err(ConfigFileError::ConfigInitializationAlreadyCalled);
        }
        self.cache = cache;
