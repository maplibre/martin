...
pub fn config_doc_yaml() -> String {
    ...
    out.push_str(
        "The shape of this file is described by the JSON Schema at \
         [`schemas/config.json`](https://github.com/maplibre/martin/blob/main/schemas/config.json) \
         — point your editor at it (e.g. via the [Red Hat YAML extension] for VS Code) for \
         live validation and autocomplete.\n\n",
    );
    out.push_str("[Red Hat YAML extension]: https://marketplace.visualstudio.com/items?itemName=redhat.vscode-yaml\n\n");
    out.push_str("```yaml title=\"config.yaml\"\n");
    // The `# yaml-language-server: $schema=…` directive lets users who copy
    // this YAML straight into a `config.yaml` get the same validation +
    // autocomplete as if they pointed their editor at the schema by hand.
    out.push_str(
