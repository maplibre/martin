...
fn binary_command(env_var: &str, name: &str) -> Command {
    if let Ok(bin) = env::var(env_var) {
        ...
        // CI points this at a relative path, meaning it relative to the workspace root - not
        // relative to whatever directory cargo happens to run the test binary from, which is
        // unstable across platforms and even between local runs and CI. A bare name like
        // `docker` has no separator and is left for PATH lookup.
        let program = if program.contains(['/', '\\']) && Path::new(program).is_relative() {
            workspace_root().join(program).into_os_string()
        } else {
