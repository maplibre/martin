...
async fn auto_configured_minimal() {
    ...
    // `rendering` is reported by a binary compiled with the feature, which the debian package
    // and the other packaged builds each decide for themselves.
    assert!(
        settings == serde_json::json!({}) || settings == serde_json::json!({"rendering": false}),
        "unexpected catalog settings: {settings}"
    );
