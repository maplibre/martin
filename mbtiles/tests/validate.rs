...
async fn normalized_with_view_tables_accept_int_type() {
    ...
    // Use INT-containing types like BIGINT, SMALLINT, TINYINT (as may be generated by some tools)
    conn.execute(
        "CREATE TABLE tiles_shallow (
             zoom_level bigint,
             tile_column smallint,
             tile_row tinyint,
             tile_data_id bigint,
             PRIMARY KEY(zoom_level, tile_column, tile_row));",
    )
    ...
    conn.execute(
        "CREATE TABLE tiles_data (
             tile_data_id bigint PRIMARY KEY,
             tile_data blob);",
    )
    .await
    .unwrap();
...
#[actix_rt::test]
#[tracing_test::traced_test]
async fn planetiler_fixture_detected_as_normalized_with_view() {
    let mbtiles = Mbtiles::new(":memory:").unwrap();
    let mut conn = mbtiles.open().await.unwrap();
