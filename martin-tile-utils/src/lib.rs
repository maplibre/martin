...
pub fn xyz_to_bbox(zoom: u8, min_x: u32, min_y: u32, max_x: u32, max_y: u32) -> [f64; 4] {
    let web_mercator = xyz_to_bbox_webmercator(zoom, min_x, min_y, max_x, max_y);

    let (min_lng, min_lat) = webmercator_to_wgs84(web_mercator[0], web_mercator[1]);
    let (max_lng, max_lat) = webmercator_to_wgs84(web_mercator[2], web_mercator[3]);
