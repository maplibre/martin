...
test-schemas:
    ...
    fixtures=(
        integration-tests/tests/snapshots/save_config__every_discovered_source_and_resource_is_spelled_out.snap
        integration-tests/tests/snapshots/save_config__every_documented_setting_survives_the_round_trip.snap
    )
...
bless-e2e *args: fetch start (cargo-install 'cargo-insta')
    ...
    cargo insta test --accept --package martin-integration-tests --features test-pg {{args}}
...
test-ssl: start-ssl (test-cargo "--all-targets") test-pg test-doc
    cargo build --package martin --package mbtiles
    cargo test --package martin-integration-tests --features test-pg
