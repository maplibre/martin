//! What `--save-config` writes down: the sources and resources martin found by itself, and the
//! settings it read from a config file.
...
/// A config that sets every documented section outside of `postgres`, which
/// `postgres.rs` covers with a database at hand.
const CONFIG: &str = "
keep_alive: 75
cache:
  size_mb: 8
