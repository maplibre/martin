...
A hillshade is the grey relief image a map multiplies over its basemap to make terrain look three-dimensional.
Martin can bake one from a source that serves *normal maps* in Mapzen's encoding - tiles whose pixels store which way the ground faces instead of a colour - which **spares the client this work** and lets the result be **compressed further**.
...
`auto` bakes with every default, a map of settings (f.ex. `convert_to_hillshade: {altitude: 60}`) overrides said defaults, and `disabled` (the default for `convert_to_hillshade`) is the same as omitting the key.
...
!!! warning "This is an expensive feature"
    Baking costs tens of milliseconds of CPU per tile, which makes hillshading an expensive feature.
    If you need this, this means you should take care in how you configure the CDN in front.
    A hillshade for fixed settings never changes (+- data changes), so it is near-perfectly cacheable at the edge.

    So in essence, this option trades server CPU time for flexibility.
    Leave it off unless callers actually need it.
