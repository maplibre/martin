...
Unlike file sources, passthrough sources have no `paths:` to glob.
Each source names an upstream directly.
...
Passthrough sources are only available via the [configuration file](config-file/index.md).
