...
Given a directory with SVG images, Martin will generate a sprite -- a JSON index and a PNG image, for both low and high-resolution displays.
...
The spritesheet generation is cached and the size of this cache can be configured via `sprites.cache.size_mb` in the config file.
...
how [MBTiles and PMTiles](config-file/index.md) are configured.
