...
### validate
If the `.mbtiles` file is of [flat_with_hash](52-schemas.md#flat-with-hash) or [normalized](52-schemas.md#normalized) type, then verify that the data stored in columns `tile_hash` and `tile_id` respectively are MD5 hashes of the `tile_data` column.
```shell
