...
You can add a layer to the map and specify Martin [TileJSON](https://github.com/mapbox/tilejson-spec) endpoint as a vector source URL. You should also specify a `source-layer` property. For [Table Sources](./table-sources.md) it is `{table_name}` by default.
...
You can also combine multiple sources into one source with [Composite Sources](./composite-sources.md). Each source in a composite source can be accessed with its `{source_name}` as a `source-layer` property.
