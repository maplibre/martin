...
mkdir -p ~/martin-ssl-test && cd ~/martin-ssl-test
...
docker run -d --name postgres-ssl \
    ...
    -p 5432:5432 \
    -v $(pwd)/server.pem:/etc/ssl/certs/server.pem:ro \
    -v $(pwd)/server-key.pem:/etc/ssl/private/server-key.pem:ro \
    postgis/postgis:16-3.5 \
    postgres -c ssl=on \
    -c ssl_cert_file=/etc/ssl/certs/server.pem \
    -c ssl_key_file=/etc/ssl/private/server-key.pem

# Test with psql
PGSSLROOTCERT=ca.pem psql "postgresql://postgres:password@localhost:5432/testdb?sslmode=verify-ca"

# Configure Martin
export PGSSLROOTCERT=$(pwd)/ca.pem
export DATABASE_URL="postgresql://postgres:password@localhost:5432/testdb?sslmode=verify-ca"
martin
```
