...
Martin requires at least one PostgreSQL [connection string](pg-connections) or a [tile source file](sources-files) as a command-line argument. A PG connection string can also be passed via the `DATABASE_URL` environment variable.
