...
if [[ "$MBTILES_BIN" != "-" ]]; then
  ...
  $MBTILES_BIN summary --format json-pretty ./tests/fixtures/mbtiles/world_cities.mbtiles 2>&1 | cleanup_json | tee "$TEST_OUT_DIR/summary.pretty.json"
  $MBTILES_BIN summary --format json ./tests/fixtures/mbtiles/world_cities.mbtiles 2>&1 | cleanup_json | tee "$TEST_OUT_DIR/summary.json"
