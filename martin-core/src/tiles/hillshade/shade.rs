...
mod tests {
    ...
    fn bakes_a_varied_field_to_the_pinned_reference_bytes() {
        ...
        assert_eq!(samples, vec![163, 163, 70, 102, 116, 116, 163, 116]);

        let no_banding = BakeParams {
            toon_bands: 0.0,
            ..BakeParams::default()
        };
        let baked = bake_with_light(&canvas, 64, &no_banding, light);
        let samples: Vec<u8> = idx.iter().map(|&i| baked.gray[i]).collect();
        assert_eq!(samples, vec![159, 159, 64, 104, 112, 118, 159, 112]);
    }
