...
fn rewrite_style(original: &Path) -> (NamedTempFile, PathBuf) {
    let body = read_to_string(original).expect("read style");
    ...
    write(tmp.path(), rewritten).expect("write rewritten style");
    let path = tmp.path().to_path_buf();
    (tmp, path)
}

#[rstest]
#[case::maplibre_demo_png("maplibre_demo.json", (0, 0, 0), ImageFormat::Png, "png")]
#[case::maplibre_demo_zoom1_png("maplibre_demo.json", (1, 0, 0), ImageFormat::Png, "png")]
#[case::maptiler_basic_png("src2/maptiler_basic.json", (0, 0, 0), ImageFormat::Png, "png")]
#[case::maplibre_demo_jpeg("maplibre_demo.json", (0, 0, 0), ImageFormat::Jpeg, "jpeg")]
#[case::maplibre_demo_zoom1_jpeg("maplibre_demo.json", (1, 0, 0), ImageFormat::Jpeg, "jpeg")]
#[case::maptiler_basic_jpeg("src2/maptiler_basic.json", (0, 0, 0), ImageFormat::Jpeg, "jpeg")]
#[tokio::test]
async fn render_tile_with_fixtures(
    ...
    let pixels: HashSet<_> = rendered_img.pixels().copied().collect();
    assert!(
        pixels.len() > 1,
    ...
    let encoded_img = if format == ImageFormat::Jpeg {
        DynamicImage::ImageRgb8(DynamicImage::ImageRgba8(rendered_img.clone()).to_rgb8())
    } else {
        DynamicImage::ImageRgba8(rendered_img.clone())
    };
    let mut rendered_buf = Cursor::new(Vec::new());
    ...
    let reference_bytes = read(&reference_path).unwrap_or_else(|_| {
        create_dir_all(reference_path.parent().unwrap()).unwrap();
        assert!(
            rendered_bytes.len() > 1000,
            "Refusing to bless suspiciously small image ({} bytes)",
            rendered_bytes.len()
        );
        write(&reference_path, &rendered_bytes).unwrap();
        panic!(
            "Created new reference image at {reference_path:?}. Commit this file and re-run the test."
        );
    });

    let rendered_for_cmp = load_from_memory_with_format(&rendered_bytes, format)
        .unwrap()
        .to_rgba8();
    let reference_for_cmp = load_from_memory_with_format(&reference_bytes, format)
        .unwrap()
        .to_rgba8();

    let similarity = rgba_hybrid_compare(&reference_for_cmp, &rendered_for_cmp)
        .unwrap_or_else(|e| panic!("image_compare failed: {e}"));

    // Identical = 1.0; JPEG is lossy so we accept a lower bound.
    let min_similarity = if format == ImageFormat::Jpeg {
        0.95
    } else {
        0.99
    };
