...
|--------|--------------------------------------------------------|---------------------------------------------------------|
| `GET`  | `/`                                                    | [List of all sources](#source-list)                     |
| `GET`  | `/{schema}.{name}`                                     | [Source TileJSON](#table-source-tilejson)               |
| `GET`  | `/{schema}.{name}/{z}/{x}/{y}`                         | [Source Tiles](#table-source-tiles)                     |
| `GET`  | `/{schema1}.{name1},...,{schemaN}.{nameN}`             | [Composite Source TileJSON](#composite-source-tilejson) |
| `GET`  | `/{schema1}.{name1},...,{schemaN}.{nameN}/{z}/{x}/{y}` | [Composite Source Tiles](#composite-source-tiles)       |
...
You can add a layer to the map and specify Martin [TileJSON](https://github.com/mapbox/tilejson-spec) endpoint as a vector source URL. You should also specify a `source-layer` property. For [Table Sources](#table-sources) it is `{schema_name}.{name}` by default.
...
map.addLayer({
  ...
  source: {
    type: 'vector',
    url: 'http://localhost:3000/public.points'
  },
...
map.addSource('rpc', {
  type: 'vector',
  url: `http://localhost:3000/public.function_source`
});
map.addLayer({
  id: 'points',
  type: 'circle',
...
map.addSource('points', {
  type: 'vector',
  url: `http://0.0.0.0:3000/public.points1,public.points2`
});
...
```js
L.vectorGrid
  .protobuf('http://localhost:3000/public.points/{z}/{x}/{y}', {
...
const pointsLayer = new MVTLayer({
  data: 'http://localhost:3000/public.points', // 'http://localhost:3000/public.table_source/{z}/{x}/{y}'
...
A list of all available sources is available at the root - `/`
...
Table Source [TileJSON](https://github.com/mapbox/tilejson-spec) endpoint is available at `/{schema_name}.{table_name}`.

For example, `points` table in `public` schema will be available at `/public.points`

```shell
curl localhost:3000/public.points
```
...
```shell
curl localhost:3000/public.points.geom
```

### Table Source Tiles

Table Source tiles endpoint is available at `/{schema_name}.{table_name}/{z}/{x}/{y}`

For example, `points` table in `public` schema will be available at `/public.points/{z}/{x}/{y}`

```shell
curl localhost:3000/public.points/0/0/0
```

In case if you have multiple geometry columns in that table and want to access a particular geometry column in vector tile, you should also specify the geometry column in the table source name

```shell
curl localhost:3000/public.points.geom/0/0/0
```
...
Composite Source [TileJSON](https://github.com/mapbox/tilejson-spec) endpoint is available at `/{schema_name1}.{table_name1},...,{schema_nameN}.{table_nameN}`.

For example, composite source for `points` and `lines` tables in `public` schema will be available at `/public.points,public.lines`

```shell
curl localhost:3000/public.points,public.lines
```

### Composite Source Tiles

Composite Source tiles endpoint is available at `/{schema_name1}.{table_name1},...,{schema_nameN}.{table_nameN}/{z}/{x}/{y}`

For example, composite source for `points` and `lines` tables in `public` schema will be available at `/public.points,public.lines/{z}/{x}/{y}`

```shell
curl localhost:3000/public.points,public.lines/0/0/0
```
...
curl \
  ...
  --get localhost:3000/public.function_source/0/0/0
```
...
Function Source [TileJSON](https://github.com/mapbox/tilejson-spec) endpoint is available at `/{schema_name}.{function_name}`

For example, `points` function in `public` schema will be available at `/public.points`
...
Function Source tiles endpoint is available at `/{schema_name}.{function_name}/{z}/{x}/{y}`

For example, `points` function in `public` schema will be available at `/public.points/{z}/{x}/{y}`
